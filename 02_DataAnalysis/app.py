...
kospi_dict, kosdaq_dict, af_dict, nyse_dict = {}, {},{},{}
...
def before_first_request():
    ...
    af = pd.read_excel('./static/data/미국상장기업리스트.xlsx', dtype={'Ticker': str})
    for i in af.index:
        af_dict[af['Ticker'][i]] = af['Company'][i]
...
def stock():
    ...
    if request.method == 'GET':
        return render_template('stock.html', menu=menu, weather=get_weather_main(),
                                kospi=kospi_dict, kosdaq=kosdaq_dict, af = af_dict,nyse = nyse_dict)
    else:
        ...
        if market == 'KS':
            ...
            code = request.form['af_code']
            company = af_dict[code]   
        else:
